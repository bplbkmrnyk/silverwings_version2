...
function ProductCard({ product }: { product: any }) {
  ...
  // Typing animation effect for the retro terminal feel
  useEffect(() => {
    if (isHovered && !isHoveringCTA) {
      let currentIndex = 0;
      setTypedText('');
      const interval = setInterval(() => {
        if (currentIndex <= product.specs.length) {
          setTypedText(product.specs.slice(0, currentIndex));
          currentIndex += 2; // Types 2 characters at a time for speed
        } else {
  ...
  }, [isHovered, isHoveringCTA, product.specs]);
  ...
  return (
    <>
      <article className="bg-surface-container-low relative flex flex-col overflow-hidden ghost-border group cursor-crosshair h-auto md:h-[400px]">
        {/* Mobile Layout (Stacked) / Desktop Layout (Absolute Fill) */}
        <div 
        ...
        >
          ...
          {/* Desktop Default Overlay (Hidden on Mobile) */}
          <div className="hidden md:flex absolute inset-0 bg-gradient-to-t from-surface-container-lowest via-surface-container-lowest/40 to-transparent flex-col justify-end p-6 md:group-hover:opacity-0 transition-opacity duration-500 pointer-events-none">
          ...
          {/* Desktop Hover CTA - Only shown on hover, at the bottom */}
        ...
        <div className="p-6 flex flex-col md:hidden bg-surface-container-low border-t ghost-border">
          ...
          <div className="p-4 bg-surface-container-lowest border border-primary/20 mb-6">
      ...
      {/* Desktop Hover Tooltip attached to pointer */}
      {isHovered && (
        <div 
